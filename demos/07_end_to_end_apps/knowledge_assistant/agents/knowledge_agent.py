...
class KnowledgeAgent:
    ...
    def initialize(self) -> None:
        ...
        if self.vector_store_id is None:
            ...
            try:
                files = list(self.client.vector_stores.files.list(vector_store_id=self.vector_store_id))
    ...
    def list_files(self) -> list[dict]:
        ...
        if not self.is_ready:
            return []
        try:
            vs_files = list(self.client.vector_stores.files.list(vector_store_id=self.vector_store_id))
            result = []
            for vsf in vs_files:
                try:
