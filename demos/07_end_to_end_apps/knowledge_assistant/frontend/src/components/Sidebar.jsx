...
import { listKBFiles, deleteFile, deleteKnowledgeBase } from '../api'
...
export default function Sidebar({
  ...
  const [deletingKB, setDeletingKB] = useState(null) // kb name being deleted
  ...
  const handleDeleteKB = async (e, kbName) => {
    e.stopPropagation()
    if (!confirm(`Delete knowledge base "${kbName}" and all its documents?`)) return
    setDeletingKB(kbName)
    try {
      await deleteKnowledgeBase(kbName)
      onDeleteKB(kbName)
      setExpandedKBs((prev) => { const next = { ...prev }; delete next[kbName]; return next })
      setKBFiles((prev) => { const next = { ...prev }; delete next[kbName]; return next })
      if (uploadTarget === kbName) setUploadTarget('')
    } catch (err) {
      console.error(err)
    } finally {
      setDeletingKB(null)
                  ...
                  <div key={kb.name}>
                    <div className="kb-item" onClick={() => onToggleKB(kb.name)}>
                      ...
                      <button
                        ...
                        onClick={(e) => handleDeleteKB(e, kb.name)}
                        disabled={deletingKB === kb.name}
